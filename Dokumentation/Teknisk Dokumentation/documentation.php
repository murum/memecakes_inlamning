<!DOCTYPE html>
<html lang="sv">
<head>
	<meta charset="UTF-8">
	<title>Dokumentation för memecak.es</title>
	<link rel="stylesheet" type="text/css" href="http://yui.yahooapis.com/3.10.1/build/cssreset/cssreset-min.css">
	<link rel="stylesheet" type="text/css" href="css/style.css">
</head>
<body>
	<div class="container">
		<h1>Memecakes dokumentation</h1>
		<div class="menu">
			<ul>
				<li><a href="./">Start</a></li>
				<li><a class="active" href="documentation.php">Dokumentation</a></li>
				<li><a href="about.php">Om Memecakes Doc</a></li>
			</ul>
		</div>
		<div class="content">
			<h2>Dokumentation för <a target="_blank" href="http://memecak.es">memecak.es</a></h2>
			<h3>Wordpress temat</h3>
			<ul class="documentation-files">
				<li><a href="pages/404.php">404.php</a></li>
				<li><a href="pages/archive.php">archive.php</a>
					<ul>
						<li>
							<a href="pages/archive-memecake.php">archive-memecake.php</a>
						</li>
					</ul>
				</li>
				<li><a href="pages/author.php">author.php</a></li>
				<li><a href="pages/footer.php">footer.php</a></li>
				<li><a href="pages/functions.php">functions.php</a></li>
				<li><a href="pages/header.php">header.php</a></li>
				<li><a href="pages/memecakes-startpage.php">memecakes-startpage.php</a></li>
				<li><a href="pages/memecakes-top.php">memecakes-top.php</a></li>
				<li><a href="pages/memecakes-upload.php">memecakes-upload.php</a></li>
				<li><a href="pages/screenshot.php">screenshot.png</a></li>
				<li><a href="pages/single-memecake.php">single-memecake.php</a></li>
				<li><a href="pages/style.php">style.css</a></li>
				<li><a href="pages/taxonomy-meme.php">taxonomy-meme.php</a></li>
			</ul>
			<h3>Wordpress temats javascript</h3>
			<ul>
				<li><a href="pages/js/upload.php">js/upload.js</a></li>
				<li><a href="pages/js/auth.php">js/auth.js</a></li>
				<li><a href="pages/js/all_sites.php">js/all_sites.js</a></li>
			</ul>
		</div>
		<div class="sidebar"><?php include('includes/sidebar.php'); ?></div>
	</div>
</body>
</html>